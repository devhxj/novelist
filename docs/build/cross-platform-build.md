...
- `Novelist.App` 发布输出；
- `frontend/dist` 前端资源；
- LibGit2Sharp 及其 NuGet 原生运行时资产，用于每本小说的本地版本历史；
...
| Git | 使用 LibGit2Sharp + NuGet 原生运行时资产，不依赖系统 `git` 可执行文件 | 避免用户额外安装 Git CLI，并保持版本历史功能随应用可用 |
...
- Git：本地版本历史通过 `LibGit2Sharp` 和 `LibGit2Sharp.NativeBinaries` 提供，`dotnet publish` 会把对应 RID 的 libgit2 原生资产放入 `runtimes/{rid}/native/`。运行时不解析系统 PATH，也不需要 `runtime/git`。
...
不需要。版本历史由 LibGit2Sharp 调用随包发布的 libgit2 原生库完成，不依赖系统 `git`、`git.exe`、`NOVELIST_GIT_PATH` 或 PATH。
