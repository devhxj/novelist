...
- Windows 下弹系统原生目录选择对话框
- macOS/Linux 预填 `~/.goink`，可直接确认
- 点击「开始使用」→ `App.Initialize(dataDir)` → 切换到 NovelListView
...
- 右上角：设置图标（齿轮），Windows 原生标题栏按钮（最小化/最大化/关闭）
- 右下角：GitHub 图标链接（小、低调）
...
设置在 NovelListView 右下角，一个小 GitHub 图标（16px），hover 显示 `github.com/xxx/goink`。或在设置面板中显示。
...
主色:      #3B82F6（蓝，按钮/选中）
文字:      #1A1A2E（标题）/ #6B7280（正文）
边框:      #E5E7EB
hover:     #F3F4F6
Muted:     已读/禁用状态用灰色降低饱和度
```
