...
- 未知授权的参考锚点现在会在材料搜索/库预览中默认截断精确原文；完整导入文本仍保留在本地 SQLite 中用于溯源、改写、材料绑定和审计。
...
- 配置 Embeddings 且参考锚点向量索引就绪时，参考材料搜索和蓝图材料绑定会把向量相似度纳入 `embedding` 评分组成；向量查询失败时仍会回退到原有词面/标签排序。
