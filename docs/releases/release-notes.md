...
- 参考数据材料化改为严格逐章顺序处理：每章完成大模型提取、embedding、向量索引和提交后才推进下一章，不再提供 5/10 章批次配置或批内并行。
- 材料化 schema 升至 v4：旧 v2/v3 使用 copy-first 重建派生表；已是 v4 但缺少单章调度列的数据库只原地增加 nullable 列，不重建、不丢 run 或材料数据。批次/retry、重复阶段字段和无消费者调度元数据已删除；章节失败时只清理当前章的未提交材料，已完成章节保持可续跑。
- “运行全部”复用当前 run/generation，跳过已完成章节并从首个未完成章节继续；章节行也可强制只运行所选章节，完成后停在 paused，不隐式处理后续章节。
- 整章 embedding 不再受旧的 64 条句子批量上限约束；embedding 输入/请求异常稳定归类为 `materialization_embedding_request_failed`，不会误报为 LLM 请求失败。

### 2026-07-12

- `素材库`添加参考书时现在只登记和校验来源，不再同步执行旧的整书机械切分与全量向量化；登记完成后可直接确认章节边界并启动材料化。
- 删除参考书、自动章节分析和 AI 蓝图预演不会再被桌面端固定 30 秒请求时限中断；长模型调用会保留清晰的进行中状态和失败反馈。
- `素材库`的大模型材料判断现在使用严格工具结果约束；DeepSeek 等带思考输出的模型不会再因健康检查输出过短被误判为不可用。
...
- 自动章节分析现在会正确处理 UTF-8 BOM 及正文标题中混用的全角/半角空白；电子书目录不再因字符编码或标题排版差异导致切分失败。
- 大模型识别出的章节模板会由原文重新验证并生成可追溯证据位置，不会因长文本中的近似字符计数误判为分析失败。
- 候选构建会把可与前后文衔接的低信息短段合并为完整语义窗口；孤立短句仍进入大模型判断，不会仅因长度被规则丢弃。
- 由多个连续句节点完整覆盖的超长自然段会拆为不重叠、可回溯的窗口，每个窗口保持在技术长度上限内，不会截断来源正文。
- 无法沿完整句节点安全拆分的超长来源会明确使本次材料化失败，不会截断、跳过或激活部分材料。
