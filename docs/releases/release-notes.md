...
- 轻量化聚焦方案（2026-08-31 定稿）落地：活动栏收敛为书架/素材库/设置三主区加本书工具；语料区重建为总览/制作/浏览/语料包四视图；拼装线（蓝图迭代、保真拼装、编排运行、插入审计、license/相似度闸门）整体退役，数据留档，相关桥接方法、MAF 工具与前端面板一并移除。
- 对话写作闭环首版：带章号的聊天回合自动检索注入 top-5 语料并以工具事件 + `corpus_usage` 双通道展示用量卡片；新增章节覆盖度信号（细纲 beat 走同一检索通路，<50% 提示「语料不足」，可直写或补书）；AI 访谈以选择题推进，作者确认后开写。
- 完成 2026-09-02/03 用户视角评审（round 3/4）修复：迁移 UI 与进度事件、章节删除安全、toast 可访问性、busy 锁、搬迁重试、大纲护栏、实时轮次快照、审批卡死升级等。

### 2026-07-12

- `素材库`添加参考书时现在只登记和校验来源，不再同步执行旧的整书机械切分与全量向量化；登记完成后可直接确认章节边界，再按 5 或 10 章批次启动材料化。
