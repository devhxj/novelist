...
>
> 2026-07-20 整章材料化重构已定案：确认后的完整章节是唯一模型输入，不再持久化 sentence/paragraph/scene、text node、候选窗口或候选复核；大模型直接逐章产出原文材料，向量模型完成全部材料索引后才原子激活 generation。生产路径不截断、不切窗口、不降级、不部分成功，异常直接失败。素材库预演与章节默认写作必须共同消费 active materials，旧 corpus 路径最终物理删除。实施范围和完成定义以[参考数据整章材料化重构计划](./materialization-quality-plan.md)为准；本文件中与其冲突的旧 text-node/materialization 设计仅描述历史实现，不再构成目标约束。
...
7. **AI 不自由补写正文**：结构化 AI 可用于目标解析、语料分析、召回融合、蓝图编排、槽位约束推理和受审计过渡，但不得脱离启用语料库、选定蓝图和可追溯证据自由生成叙述。fake/deterministic 实现只能作为测试替身；生产材料化必须使用已配置的大模型和向量模型，失败必须报错，不能作为运行时 fallback。

以上是不随里程碑裁剪而改变的产品不变量。M1 可以薄，但必须验证“跨库召回 → 多蓝图反馈循环 → 选定蓝图 → 最大保真正文候选”的最小闭环；M3+ 不能把检索和蓝图重新简化为单 anchor、单蓝图或自由补写路径。
...
-- technique_abstract/material 向量必须由已配置 embedding 模型生成并进入必需向量索引；
-- embedding 模型、维度、向量数量或索引不可用时处理/检索失败并返回错误；
-- JSON 扫描、词法结果或旧模型向量不能作为生产降级路径。
```
...
**材料化边界修正（2026-07-22）**：下面的 text node 分层是旧实现说明，不再是目标材料化架构。新链路只保留参考书、确认章节边界、逐章运行进度、材料、向量和 active generation；单 worker 按 boundary 顺序读取完整章节，一章一次调用大模型，并要求返回非空、可在原章中精确找到的材料正文。每章完成提取、embedding、索引和事务提交后才推进下一章。任一模型、章节、校验或索引失败即 run 失败，不切窗口、不截断、不跳章、不降级、不复活失败 run。详见[参考数据整章材料化重构计划](./materialization-quality-plan.md)。
...
**素材库工作台边界（2026-07-11 截图复核）**：素材库不是空白侧栏加通用聊天。左侧应是参考书籍管理区，提供真实列表、筛选、添加、删除、状态与选择；中间是 ready 来源的六维语料覆盖和按需检索（材料类型、叙事功能、情绪张力、场景节点、叙事视角、表达技法）；右侧是仅供比较的蓝图预演。预演必须把左侧显式选择的 ready 参考书传为 `include_anchor_ids`，展示候选、节拍、覆盖度和来源分布，但不创建持久写作 session、不取代章节默认跨库检索、不生成正文候选，也不写入编辑器。该修正属于已复现的核心任务可用性问题，允许在冻结专家控制面的前提下收口。
...
原子任务继续使用 `[x]` / `[ ]`，但复选框只回答“这条任务是否已有某种实现与定向验证”，不能回答“整个里程碑是否可用于真实生产”。从本次治理起，M0-M9 统一使用 `N/S/P/L` 四级成熟度；升级必须有对应证据，不能因演示路径跑通、UI 已接入、测试数量增加或原子任务全部勾选而自动升级。

| 代码 | 状态 | 定义 | 最低证据 | 禁止误读 |
|---|---|---|---|---|
| `N` | **未开始** | 尚无可执行纵向路径，或只有设计/契约占位 | 无 | 不能用“已建表/已定义接口”替代可运行能力 |
| `S` | **薄切片完成** | 在受控 fixture、fake model 或有限路径上证明关键契约和算法可工作 | 定向测试、golden 小样、明确适用边界 | 不等于后台运行、不等于真实长篇效果、不等于用户可持续使用 |
...
5. **P4 加深 M4/M5 写作效果并完成 M9 真实走查**：持久 blueprint session、单一章节默认路径、错误恢复和视口/键盘自动检查已完成；先以 `test:reference-workspace` 验收素材库的参考书管理和临时蓝图预演，再证明同一目标的多份蓝图在 beat、来源组合、节奏/情绪策略和 gap 处理上有可度量差异，并验证选定蓝图内的最大保真拼装、受控槽位/过渡和多正文候选，以及至少 5 名目标用户中的 4 名可无指导完成主流程。**停止继续堆 M5 审计规则和专家控件**，除非新增项来自已复现的安全漏洞、授权绕过、真实语料评测失败或核心任务可用性失败，并附失败 fixture/workflow。
...
| M9 | `8/10`，`S，章节自动路径已收口` | 素材库参考书籍管理和蓝图预演的专用浏览器验收，以及真实目标用户无指导完成率、完成时间、回退次数、失败点和主观难度记录 |

依赖顺序固定为：`P0 绿色基线（已通过）→ M2 真实时限与 50K（已通过）→ M9 章节默认体验（已通过）和素材库工作台浏览器验收（进行中）→ 效果/真实用户证据 → M3 标定 → M4/M5 写作效果`。视觉收口从 P0 就随每个主线切片验证，不等到最后统一“美化”。
