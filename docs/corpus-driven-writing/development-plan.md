...
**素材库工作台边界（2026-07-11 截图复核）**：素材库不是空白侧栏加通用聊天。左侧应是参考书籍管理区，提供真实列表、筛选、添加、删除、状态与选择；中间是 ready 来源的六维语料覆盖和按需检索（素材类型、叙事功能、情绪机制、场景节拍、视角、技法）；右侧是仅供比较的蓝图预演。预演必须把左侧显式选择的 ready 参考书传为 `include_anchor_ids`，展示候选、节拍、覆盖度和来源分布，但不创建持久写作 session、不取代章节默认跨库检索、不生成正文候选，也不写入编辑器。该修正属于已复现的核心任务可用性问题，允许在冻结专家控制面的前提下收口。
