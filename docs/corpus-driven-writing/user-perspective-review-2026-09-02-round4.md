...
### 残余事项（2026-09-03 全部处置完毕）

原列四项，逐项收口如下：

1. **迁移后陈旧 UI → 已修复**：迁移成功提示展示约 1.2 秒后整页刷新，应用以新目录重新初始化；不再存在"旧界面对着已切换数据源继续写"的窗口。工作流断言刷新后工作区正常加载（桥调用参数在 reload 前取证）。
2. **迁移无进度 → 已修复**：`RelocateAsync` 接受进度回调（先清点总文件数，复制中上报累计值）；后端经 `IBridgeEventSink` 发 `datadir:migration:progress` 事件（进度发射 best-effort，绝不使迁移失败、不随请求取消）；设置页迁移表单实时显示"已复制 X / Y 个文件"。集成测试断言进度单调递增且末次为最终复制数；工作流经确定性进度事件断言 UI 呈现。
3. **F13 第二档（60+ 未接线方法）**：维持"专门批次"排期不变——这不是缺陷修复而是路线决策（接线以作者动线准入 / 确认不进路线则整体删除），且须与行为修复分开提交；处置原则已在 F13 节写明。
4. **审批后端会话死亡检测 → 前端侧已收口，后端明确不做**：等待 60 秒无进展时卡片把提示升级为"后端本轮可能已失效，建议结束本轮后重试"（`role="alert"` + 结束本轮入口）；后端**不引入自动击杀**——`timeoutMs: null` 下的长审批是合法形态，自动终止误杀的代价高于作者手动结束的成本（与"不做全自动材料化"同一决策逻辑）。计时随等待期（status×submitting 键）自动复位。

另闭环轮 3 遗留的**计划镜像懒迁移**：`GetChapterPlansAsync` 读取时检测"有内容的槽位缺镜像文件"并幂等补写（镜像失败不影响读取）；存量小说不再需要等首次保存才出现 plans/ 镜像。集成测试覆盖补写与幂等。

**仍然开放的是功能路线而非缺陷**（均有既定排期，见第三轮第三节）：F7 二批（素材检索/详情接线）、scene/trope 分析管线启用、切分分析后台 job 化、跨设备语料包、F6 真实用户验证。
