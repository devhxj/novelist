...
> **2026-07-22 整章材料化口径：** 当前实现不再建设候选窗口、候选准入、人工边界复核、旧库 backfill 或多路降级检索；确认后的完整章节由大模型一次提取材料，每章材料完成向量化和索引提交后才顺序推进，异常直接失败。M1/M2 中旧 text-node/corpus 管线的历史勾选不再代表当前材料化路径；实现边界和验收证据见[参考数据整章材料化重构计划](./materialization-quality-plan.md)。
...
**材料化重构状态：** 已实施。当前路径为整章输入、每章一次模型提取、双模型硬依赖、无切分/截断/降级/部分成功、单 worker 严格逐章提交和 active generation 原子发布；旧 node/candidate/corpus 路径及批次/retry 实现已物理删除。失败后可在同一 run/generation 上“运行全部”跳过已完成章节继续，也可强制只运行指定章节。
