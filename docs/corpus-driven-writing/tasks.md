...
> **轻量化聚焦退役标注（2026-08-31）：** 依据 [lightweight-refocus-proposal-2026-08-31.md](./lightweight-refocus-proposal-2026-08-31.md)，拼装线（M4 蓝图迭代、M5 保真拼装、蓝图预演、anchored draft、编排运行、插入审计、license/相似度闸门、adapt/audit-reuse）的前后端代码已物理退役：蓝图/拼装/审计服务、bridge 方法、MAF 工具、前端面板与工作流测试均已删除；SQLite 表与数据一律留档未删。M4/M5 及 M6 插入审计条目自此视为**已退役**（历史状态快照保留，不再排期）；M6-M8 冻结政策不变，保留部分（导入/切分/分析管线、复核队列、四路召回、SafePath/审批/git 审计）继续有效。后续里程碑以提案第三节的「语料区重建 + 对话写作闭环」为准。
