...
> **Current status (2026-07-11):** The corpus-library/chapter-editor split below exists as a thin slice. The later “main-panel workflow” and “current frontend status” sections record the older reference-panel implementation for regression history. `ChapterReferencePanel` uses the persistent blueprint session for the default chapter path; automatic and expert controls are layered rather than concurrent defaults. `ReferenceCorpusWorkspace` is the current corpus-activity center, with `ReferenceBookSidebar` and a transient `BlueprintPreviewPanel`; the older `ReferenceAnchorView` is no longer mounted as the default activity route. Focused browser acceptance for the new corpus workspace and real-user evidence remain open.
...
```text
frontend/src/components/reference-anchor/ReferenceCorpusWorkspace.tsx
frontend/src/components/reference-anchor/MaterialCoveragePanel.tsx
frontend/src/components/reference-anchor/ReferenceBookSidebar.tsx
frontend/src/components/reference-anchor/BlueprintPreviewPanel.tsx
frontend/src/components/reference-anchor/ReferenceAnchorView.tsx (historical, not the default activity route)
...
- `ReferenceCorpusWorkspace.tsx` is the shared `素材库` activity center. It shows ready-source coverage through six concrete dimensions (material type, narrative function, emotion, scene, point of view, and technique) and requests paged material results only after the author supplies a tag or search condition. It must not expose current-chapter orchestration, prose generation, candidate generation, or final insertion controls in the default path.
- `ReferenceBookSidebar.tsx` is the left-side reference-book working list for the same activity. It loads `GetReferenceAnchors`, supports local file selection, create/delete, filtering, ready-state selection, and reports selection to the workspace. It replaces the former empty activity sidebar; it does not expose raw source text or add a chapter-writing entry point.
- `BlueprintPreviewPanel.tsx` replaces generic chat only while the `素材库` activity is active. It is a transient comparison tool: it requires explicitly selected ready books and calls `GenerateReferenceCorpusBlueprintCandidates` with their ids in `scope.include_anchor_ids`, then shows strategy, beats, coverage, and source distribution. It must not create or advance a persistent chapter session, generate prose candidates, insert into Monaco, or call `SaveContent`. The default chapter path remains `ChapterReferencePanel` and retains session-bound cross-library retrieval without manual book selection.
- `ReferenceAnchorView.tsx` remains as a historical implementation of the previous advanced corpus panel and is not mounted from `WorkspaceView`. It cannot be used to claim that the current default workspace exposes its import, analysis, governance, or style-profile controls.
...
- The dedicated side-panel list/filter behavior is now implemented as `ReferenceBookSidebar`; `test:reference-workspace` is the focused browser acceptance for its add/delete/select flow and for the transient preview's no-`SaveContent` boundary. Chapter candidate copy/insertion remains owned by `ChapterReferencePanel`.
