...
- The owned TypeScript bridge adapter exposes both `CreateReferenceAnchor` and `CreateReferenceAnchors`; the current UI still imports one source at a time and does not yet provide a bulk import/library-pack workflow.
