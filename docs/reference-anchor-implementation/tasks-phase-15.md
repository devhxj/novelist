...
**Status:** Open as of 2026-07-07. Task 1 is complete: Phase 15 payload contracts, compatibility method names, owned frontend adapter method declarations, representative serialization/front-back registry tests, long-running `task_id` coverage, and app/build update-check endpoint configuration are in place. Task 2 has its app-settings persistence slice implemented for Git author identity, update-check preferences, layout widths, and window bounds/maximized state. Remaining Phase 15 bridge methods intentionally stay at the compatibility boundary until their corresponding service tasks land.
