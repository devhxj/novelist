...
`frontend/src/components/reference-anchor/StyleProfileLibraryPanel.tsx` and its former reference-anchor host remain as historical Phase 14 implementation material. They are not mounted from the current default `素材库` route. The `test:reference-style` and `test:reference-style:stress` package names remain only as compatibility aliases for the current workspace workflow and must not be cited as present-day style-profile or stress evidence; the Phase 14 browser evidence above is historical.
