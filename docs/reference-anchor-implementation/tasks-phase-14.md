...
- [x] Integration tests prove candidate/source similarity findings flow through reuse audit and anchored draft audit without leaking copied source text into required fixes or readable reports.
...
- [x] Retrieval gaps and source-leak failures provide concrete next actions.
...
**Status:** Complete for the Phase 14 agent authority boundary. Agent-side `search_reference_materials` can pass style profile ids, style dimensions, and imitation intensity into the existing read/search material boundary while keeping `novel_id` runtime-injected and source/license/visibility filtering in the service. Agent-side `get_reference_style_profiles` and `get_reference_style_profile` provide read-only profile inspection with injected novel context and no source text. Agent-side `get_reference_draft_audits` and bridge `GetReferenceAnchoredDraftAudits` expose persisted draft audit reports as read-only metadata/findings with candidate-id filtering and bounded limits; they do not return candidate text, source text, prompts, paths, approvals, restore controls, or chapter-writing capability. Dedicated style-audit inspection is available through bridge `GetReferenceStyleAuditFindings` and agent `get_reference_style_audit_findings`; both read persisted audit findings for `source_leak`, `style_distance`, and `style_fit` risk types only, inject novel context, and do not return candidate text, source text, prompts, paths, approvals, restore controls, or chapter-writing capability. MAF integration coverage proves style filters do not bypass cross-novel profile or private workspace-corpus boundaries, draft audit inspection uses injected novel context with blueprint ownership validation, and dedicated style-audit inspection returns only bounded style/source-leak findings. The allowed/forbidden style-tool matrix is now explicit: agents may search/inspect style metadata and prepare candidates, but cannot build/import/archive/restore style profiles, approve style contracts, resume/approve orchestration decisions, insert candidates, or call `SaveContent`. Service-level approval now rejects `approver_origin != "user"` when a blueprint contains `style_contract`, so agent tools cannot approve style contracts even though ordinary non-style blueprint approval remains available.
...
- [x] Dedicated style-audit inspection tests for bridge routing, agent schema, injected novel context, source-leak/style-distance filtering, and absence of candidate/source text, prompt, path, approval, restore, and write fields.
- [x] Bridge/agent authority tests proving no bypass of user approvals. Material/profile visibility, draft-audit inspection, dedicated style-audit inspection authority, allowed/forbidden style-tool matrix coverage, and service-level rejection of agent approval for `style_contract` blueprints are covered.
...
- [x] The full covered style workflow is exercised by Playwright with screenshots, bridge-call assertions, console diagnostics, and stress metrics. Default style-assisted orchestration and material-gap/source-leak recovery guidance are now covered by the main reference-style browser workflow.
