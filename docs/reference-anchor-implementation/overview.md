...
- `SqliteReferenceAnchoredDraftService` can create deterministic chapter blueprints with structured logic, emotion, narration, character, reference-use, transition, and execution tracks; review them; approve passing reviews; bind approved blueprint beats to ranked reference materials; mark blueprints stale when the source chapter plan hash changes; and reject draft generation before required selected material links exist;
