...
- [x] Developer workflow expectation is finalized: keep explicit frontend build/Vite steps for faster backend-only loops; desktop launch does not build frontend assets, `npm --prefix frontend run build` prepares `frontend/dist`, and Vite debugging uses `--start-url=http://localhost:5173/`.
...
- [x] Documentation check: `README.md`, `docs/build-setup.md`, and `schema-and-integration.md` agree that desktop launch uses prebuilt `frontend/dist` or an explicit Vite `--start-url`.
...
- [x] Reference source libraries and extracted materials can exist without `novel_id` ownership.
- [x] Existing per-novel anchors migrate or are readable through a compatibility layer without losing source hashes, segment ids, material ids, user-verified tags, feedback, or audit provenance. Existing private/restricted anchors remain readable by their owner novel, explicit single/bulk promotion migrates selected owned anchors to nullable workspace-corpus ownership, and legacy workspace-visible rows with positive owners are auto-promoted; private/restricted anchors are not silently promoted because changing visibility requires user action.
...
- [x] Material search accepts story-context inputs and returns globally sourced candidates that are filtered by license/status policy and scored by beat function, emotion, POV, prose duty, technique, lexical similarity, embeddings when available, and feedback boosts.
- [x] Retrieval gaps are explicit states, not silent free-drafting fallbacks: automatic query expansion may run first, weak matches must be marked low confidence and carry elevated draft-audit risk, optional no-reuse beats require approved reasons, and source-required beats must stop for user action.
- [x] Blueprint generation no longer requires `anchor_ids`; it may accept an optional corpus search policy or explicit include/exclude anchors for advanced control.
- [x] Material binding records exact selected corpus material ids against the current blueprint `analysis_contract_hash` so later draft candidates remain auditable.
- [x] Per-novel known facts, forbidden facts, timeline state, character state, and POV boundaries still gate whether a globally sourced material may be used.
...
- [x] Migration tests from per-novel `reference_anchors` to shared corpus records or compatibility reads. Coverage includes legacy schema rebuild to nullable workspace-corpus ownership, automatic promotion of legacy positive-owner rows that already carry `corpus_visibility = 'workspace'`, explicit single-anchor plus transactional selected-anchor promotion APIs, and promotion tests that preserve source hashes, segment ids, material ids, user-verified tags, per-novel feedback scope, and reuse audit provenance.
...
- [x] Frontend smoke test for corpus management and default automatic material selection. Current coverage asserts default automatic material selection, safe `user_provided` license policy, separated corpus-library/reference-writing section headings, create/list/edit corpus metadata, batch source-path import and JSON library-pack manifest import through `CreateReferenceAnchors`, owner-scope filtering/counts, local list query/license/source-trust filters, selected-row bulk promote/archive actions, paginated row-level material browsing with local page filtering/sorting, independent material-library search without selected anchor ids, material-library current-page filtering/sorting/archive/restore, material-library cross-page selected bulk tag correction, single-material and current-page selected bulk material tag correction, material-library bulk tag correction, single-anchor promotion, and workspace-corpus archive actions.
...
- [x] Explicit per-novel anchor promotion coverage now also proves material-level user-verified tag corrections and persisted reuse candidate/audit provenance stay attached to the same material id after the anchor becomes shared workspace corpus; private/restricted anchors remain owner-scoped unless the user explicitly promotes them.
...
- [x] The reference-anchor Playwright workflow now asserts default automatic material selection UI state and bridge payload behavior: `anchor_ids` is `null`, include/exclude anchor filters are empty, `corpus_search_policy.mode` is `story_context`, `license_statuses` defaults to `["user_provided"]`, and no `SaveContent` call occurs.
...
- [x] The corpus management panel now exposes an independent `材料库` surface that searches accessible corpus materials with `anchor_ids: []`, shows score components, supports current-page filtering/sorting, preserves selected materials across material-library page navigation, and uses `UpdateReferenceMaterialsTags` for selected bulk tag correction. The Playwright workflow asserts the library search payload, filtered/empty/sorted current-page states, cross-page selection counts, corrected row display, and library-specific bulk-correction note. This is still not full material-library IA, bulk import, or automatic migration.
- [x] The material-library surface now supports selected material soft archive through `DeleteReferenceMaterials`; SQLite stores `reference_materials.archived_at`, hides archived materials from normal search/adapt/audit/tag-correction/vector-scoring paths, rolls back if any selected material is inaccessible, preserves archive markers across anchor rebuilds, and preserves source segments, material rows, links, candidates, and audit provenance. The Playwright workflow asserts cross-page material archive payloads and current-page removal. This is a selected-material policy slice, not full material lifecycle management or restore/delete administration.
- [x] The material-library surface now supports explicit archived-material browsing and restore. `SearchReferenceMaterials` accepts `archive_filter`, default searches still hide archived rows, and `RestoreReferenceMaterials` transactionally clears `reference_materials.archived_at` for selected accessible archived material ids. SQLite coverage proves restored materials become searchable/adaptable again and rollback preserves archive markers when any selected material is inaccessible; the Playwright workflow asserts archived-filter payloads, disabled archived-row bulk correction, restore payloads, and current archived-page removal. This remains selected-material lifecycle administration, not full import/library pack management or automatic migration.
...
- [x] `CreateReferenceAnchors` now provides a contract, bridge, service, and frontend-adapter entrypoint for importing 1-50 reference sources in one call. The current implementation reuses `CreateReferenceAnchor` sequentially, preserves response order and workspace-corpus nullable ownership semantics, and has SQLite coverage proving multiple workspace imports remain searchable from another novel without material-id collisions. This is a bridge/API bulk import slice, not an all-or-nothing library-pack importer or bulk import UI.
- [x] The `导入语料来源` panel now exposes a basic batch source-path importer backed by `CreateReferenceAnchors`. It accepts multiple local paths, reuses the current author/license/visibility/source-trust/tag fields, and derives ordered titles from the shared title or source filename.
- [x] The `导入语料来源` panel now supports JSON library-pack manifests as either a top-level array or `{ "sources": [...] }`; each source can override path/title/author/source kind/license/visibility/source trust/user tags, while missing fields inherit the current import metadata, and the Playwright workflow asserts the expanded `CreateReferenceAnchors` payload.
- [x] Phase 12 corpus-management UI is complete at the current implementation boundary: source import, library rows, promotion, soft archive/restore, material browsing, material tag correction, and default automatic retrieval are covered; hard delete and more opinionated library-product IA are deferred product work.
- [x] `dotnet test tests/Novelist.Tests/Novelist.Tests.csproj --filter 'CreateReferenceAnchorsPayloadUsesStableSnakeCaseJsonNames|ReferenceAnchorHandlersRouteEveryMethodToServiceOperations|CompatibilityRegistryIncludesReferenceAnchorMethods|BridgeFrontendContractTests|CompatibilityAppMethodListHasExpectedCoverage' -v minimal -p:UseSharedCompilation=false`
- [x] `dotnet test tests/Novelist.Tests/Novelist.Tests.csproj --filter 'MafToolRegistryTests' -v minimal -p:UseSharedCompilation=false`
- [x] `dotnet test tests/Novelist.IntegrationTests/Novelist.IntegrationTests.csproj --filter 'CreateAnchorsImportsWorkspaceCorpusSourcesWithoutLosingMaterialIdentity|CreateAnchorsValidatesBatchSize' -v minimal -p:UseSharedCompilation=false`
- [x] `npm --prefix frontend run test:reference-anchor`
- [x] `npm --prefix frontend run lint`
- [x] `npm --prefix frontend run build`
...
- [x] `dotnet test tests/Novelist.IntegrationTests/Novelist.IntegrationTests.csproj --filter PromotePerNovelAnchorToWorkspaceCorpusPreservesMaterialIdentityAndFeedbackScope -v minimal -p:UseSharedCompilation=false`
...
Phase 12 is complete at the current implementation boundary: shared workspace corpus storage, explicit per-novel-to-workspace promotion, story-context material retrieval, provenance-bound material binding, per-novel safety gates, feedback scoping, agent authority limits, and corpus-management UI are implemented and verified. Future work may deepen hard-delete policy, library product IA, or app-wide QA, but those are outside the Phase 12 closure boundary.
