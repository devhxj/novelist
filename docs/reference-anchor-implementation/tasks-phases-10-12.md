...
- [ ] Frontend smoke test for corpus management and default automatic material selection. Current coverage asserts default automatic material selection, create/list/edit corpus metadata, owner-scope filtering/counts, local list query/license/source-trust filters, row-level material preview, and a single-anchor promotion action; full corpus library management UI smoke remains pending.
