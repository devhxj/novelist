...
- [ ] UI provides corpus management as a library feature and reference-anchored drafting as an automatic retrieval feature, not as a per-novel setup checklist. Current coverage names and separates `语料库管理` from `参考写作检索`, adds selected-row bulk promote/archive actions, exposes an independent material-library search/filter/sort/cross-page correction/archive/restore surface that does not require selected anchors, and now has both basic batch source-path import and JSON library-pack manifest import backed by `CreateReferenceAnchors`; full library management IA, migration-management UI, and full material delete/restore administration remain pending.
