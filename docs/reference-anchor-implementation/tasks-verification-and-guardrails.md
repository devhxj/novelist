...
Latest Phase 12 material-library filter/sort slice: `npm --prefix frontend run test:reference-anchor`, `npm --prefix frontend run lint`, and `npm --prefix frontend run build` passed after the corpus management panel's dedicated `材料库` surface gained current-page filtering/sorting and visible-result selection for bulk tag correction. The mocked browser workflow asserts `SearchReferenceMaterials` is called with `anchor_ids: []` for accessible corpus-wide search, verifies score-component display, filtered/empty/sorted page states, selects the currently visible filtered material, and asserts `UpdateReferenceMaterialsTags` with `note: "corpus material library bulk correction"` updates visible rows. This is an independent material-library management surface, but it still does not cover bulk import, automatic migration, cross-page selection, material archive/delete policy, or the final full corpus library IA.
