...
Run before closing the expanded Phase 13 quality gate:
...
- Status: reopened. The existing `test:app` smoke remains baseline coverage, not the completion boundary.
- Suite boundary: `test:reference-anchor` remains the deep reference workflow, `test:app` owns baseline broad regression, and expanded Phase 13 must add full-surface, stress, and usability commands.
...
- CI policy: `npm run verify` remains a baseline command; expanded Phase 13 closure additionally requires `test:app:full`, `test:app:stress`, and `test:app:usability`.

## Recommended Next Coding Sessions

Start with the expanded Phase 13 Playwright QA gate. Do not restart from Phase 0-12 unless contracts or implemented behavior have regressed.
...
1. Implement the expanded Phase 13 full-product Playwright QA gate, including full-surface, stress, usability, and production-boundary checks.
2. Keep the Phase 11/12 orchestration and corpus gates intact while adding Phase 13 coverage.
...
2. Treat additional hard-delete policy, deeper library product IA, and global feedback tuning as future product work unless Phase 13 exposes them as high-severity usability or robustness issues.
3. Keep the existing app-wide baseline green while implementing the expanded Phase 13 quality gate.
