...
Latest Phase 12 selected-row bulk corpus actions slice: `dotnet test tests/Novelist.Tests/Novelist.Tests.csproj --filter 'PromoteReferenceAnchorsToWorkspaceCorpusPayloadUsesStableSnakeCaseJsonNames|ReferenceAnchorHandlersRouteEveryMethodToServiceOperations|CompatibilityRegistryIncludesReferenceAnchorMethods' -v minimal -p:UseSharedCompilation=false`, `dotnet test tests/Novelist.IntegrationTests/Novelist.IntegrationTests.csproj --filter PromoteAnchorsToWorkspaceCorpusPromotesOwnedRowsAtomically -v minimal -p:UseSharedCompilation=false`, and `npm --prefix frontend run test:reference-anchor` passed after the corpus library list gained selected-row bulk promotion for per-novel anchors through transactional `PromoteReferenceAnchorsToWorkspaceCorpus` and selected-row bulk archive for workspace-corpus rows. Coverage asserts stable bridge JSON, bridge routing, compatibility registration, atomic material-identity-preserving promotion, selection counts, clearing processed selections, bulk promotion payloads, archive payloads, and archived rows disappearing from the visible library. This remains a selected-row management slice, not bulk import, automatic migration, or bulk material editing.
