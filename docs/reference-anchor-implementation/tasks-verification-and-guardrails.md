...
Latest Phase 12 material-library surface slice: `npm --prefix frontend run test:reference-anchor`, `npm --prefix frontend run lint`, and `npm --prefix frontend run build` passed after the corpus management panel gained a dedicated `材料库` search/correction surface. The mocked browser workflow asserts `SearchReferenceMaterials` is called with `anchor_ids: []` for accessible corpus-wide search, verifies score-component display, selects returned materials, and asserts `UpdateReferenceMaterialsTags` with `note: "corpus material library bulk correction"` updates visible rows. This is an independent material-library management surface, but it still does not cover bulk import, automatic migration, cross-page selection, material archive/delete policy, or the final full corpus library IA.
