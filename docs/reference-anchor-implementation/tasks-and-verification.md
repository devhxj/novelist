...
- [Phase 15](tasks-phase-15.md): open `goink-master` feature merge plan for EPUB/TXT/MD import, style material library, narrative pattern extraction, Git history visualization, desktop UX persistence/update checks, and error-feedback hardening.
...
Phases 0-14 are complete at their current implementation boundaries and remain recorded for traceability and regression checks. Phase 15 is the open product-merge implementation phase from the latest `goink-master` snapshot into the current Novelist stack; it must port behavior through .NET/Photino contracts and owned TypeScript adapters rather than reviving Go/Wails/Python paths. Phase 15 Task 1 is complete for contracts, compatibility registration, frontend adapter sync, and app/build update-check configuration; Task 2 is the next storage/migration implementation gate. The expanded Phase 13 app-wide matrix and Phase 14 reference-style gates remain regression gates while Phase 15 is implemented.
