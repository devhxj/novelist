...
| RC-02 | Cover API is not implemented on the .NET product path | `INovelService` / `FileSystemNovelService` save, read, and delete validated cover images. `GetCover` now serves cover metadata and base64 data through the Photino bridge; the old `/covers/{novelId}` HTTP route has been removed with the ASP.NET Core host. | `NovelServiceTests` cover save/read/delete, invalid payload rejection, `GetCover` payload/null/too-large behavior, bridge calls, and Git commits. |
...
Local self-contained RID publish is not fully verifiable in this sandbox because network access is restricted and the local cache may not contain RID runtime packs such as `Microsoft.NETCore.App.Runtime.win-x64` and `Microsoft.WindowsDesktop.App.Runtime.win-x64`. `NO_RESTORE=1 bash scripts/novelist-publish.sh win-x64` therefore fails with `NETSDK1047` unless a RID restore has already populated assets. GitHub release jobs run restore with network access before RID publish, and the publish script preserves the previous output when this local offline failure occurs.
