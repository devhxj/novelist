...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>Phase 16 正在把共享素材库处理和章节级参考使用拆开。默认界面已分离，材料/来源处理/来源片段明细可查看，剩余重点是更完整的重试、重建和启动恢复矩阵。</td>
    </tr>
    <tr>
...
- 参考锚定界面开始按 Phase 16 信息架构收敛：`素材库`负责共享语料导入、处理、浏览、标签校正、归档恢复和风格画像；章节编辑器里的`参考素材`面板负责章节级推荐、编排、审计和显式候选插入。
- 参考来源处理记录现在能继续钻取到材料明细或来源片段明细；即使材料抽取失败、还没有 material row，也可以通过受限的片段预览查看来源片段、hash、offset 和处理记录。
- 标签校正队列由服务端 `GetReferenceMaterialTagReviewQueue` 提供，当前覆盖未校正、低置信和 unknown 标签；冲突标签只会在分析器持久化冲突信号后进入队列。
- 参考锚定的批量导入、失败导入、重试/重建、重复导入和启动恢复继续加固，目标是在自动处理素材时减少人工接入，同时不丢 provenance、人工校正或可搜索材料。

### 2026-07-07

- 书架现在可以通过桌面文件选择或拖放本地 EPUB/TXT/Markdown 文件启动小说导入；不支持的拖放内容会直接显示反馈。
- 桌面更新检查的发布端点现在由应用构建配置或启动参数提供；默认不内置 live 发布地址，自动检查保持关闭。
- 参考锚定默认编排现在可选择 active 风格画像并调整强度、拟合、证据和风险策略；该策略会进入待审批蓝图的 `style_contract`。
- 参考锚定编排遇到材料缺口、检索/溯源缺口或 source-leak 风险时，现在会给出具体恢复动作，例如补充/恢复材料、放宽检索过滤、重绑更强材料、降低模仿强度或重新生成候选。
- 参考锚定审计现在会额外拦截整体候选/来源相似度过高的改写，报告只显示相似度比例和阈值，不回显来源正文或候选正文。
- 参考锚定草稿审计现在会按模仿强度检查泛化风格质量风险，包括抽象总结、情绪直说、机械连接/三连结构和套话公式；报告不回显候选正文或来源正文。
- 参考风格画像新增 10MB 浏览器压力回归，覆盖大语料画像构建进度、画像详情和材料库分页检索。
- 风格画像库界面现在会显示画像构建进度、诊断和错误状态，支持取消运行中的构建，并可在失败或取消后刷新检查结果。
