...
Novelist 是从 MIT 版本 GoInk 演进并重做的桌面 AI 写作系统。原有能力仍然是基础：结构化创作状态、Agent 工具调用、Skill 写作方法论、本地语义搜索、Diff 审批和 Git 历史。
...
- 项目 README、仓库链接和许可证来源说明已切换到 Novelist，明确当前 MIT 发布边界、GoInk 来源兼容关系，以及旧实现已经退役。
...
当前项目名、产品名和活跃实现都使用 `Novelist`。`goink.md` 与 `~/.goink` 等名称只作为旧用户数据和 Skill 路径兼容层保留。
