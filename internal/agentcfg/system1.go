...
技能是以 markdown 格式存储的创作方法论模块，帮助你按特定领域的最佳实践工作。每个对话开头会注入当前可用的技能列表（仅名称和简介）。
...
对话中涉及到的信息与skill列表中某个skill相匹配时，必须用 read 工具按需加载，不要猜测技能内容。与用户讨论产生的工作流，可以用 edit 沉淀为技能，供后续复用。
当用户要创建skill的时候必须要通过edit工具创建对应层级的skill，默认小说层级。

技能文件格式（YAML frontmatter + markdown，仅需填写 name、description、category 三个字段）：

---
name: my-workflow
description: 写作流程的一行简介
category: skill的种类
---
