...
export function statusTone(status: string): string {
  if (status === 'approved' || status === 'material_bound' || status === 'passed' || status === 'completed') return 'text-emerald-600 dark:text-emerald-400'
  if (status === 'failed' || status === 'review_failed' || status === 'stale' || status === 'error' || status === 'cancelled') return 'text-destructive'
  if (status === 'warning' || status === 'waiting_for_user') return 'text-amber-700 dark:text-amber-300'
