  ...
  return (
    <main className="relative flex-1 min-w-0 overflow-hidden bg-[#fafbfc]">
        ...
        return (
          <div className="absolute left-5 bottom-5 z-10 w-64 rounded-lg border border-slate-200 bg-white/94 p-4 shadow-lg backdrop-blur text-sm">
            <div className="flex items-start justify-between gap-2 mb-2">
              <h3 className="font-semibold text-slate-800">{selectedCharacter.name}</h3>
              <button
            ...
            {!hasContent ? (
              ...
              <div className="space-y-3">
                ...
                {brief && (
                  <div>
                    <p className="text-xs text-slate-400 mb-1">简介</p>
                    <p className="text-xs text-slate-500 leading-relaxed">{brief}</p>
                  </div>
                )}
                {traits.length > 0 && (
                ...
                {abilities.length > 0 && (
                  <div>
                    <p className="text-xs text-slate-400 mb-1">能力</p>
                    <div className="flex flex-wrap gap-1">
                      {abilities.map((a, i) => (
                        <span key={i} className="inline-block rounded bg-slate-100 px-2 py-0.5 text-xs text-slate-600">{a}</span>
                      ))}
                    </div>
                  </div>
                )}
      ...
      <div className="absolute right-5 bottom-5 z-10 flex items-center gap-3 rounded-md border border-slate-200 bg-white/84 px-3 py-2 text-[11px] text-slate-500 shadow-sm backdrop-blur">
        ...
        <span className="inline-flex items-center gap-1.5"><span className="h-0 w-7 border-t border-dashed border-slate-300" />其他</span>
      </div>
      ...
      {error ? (
        ...
        <div className="relative z-10 flex h-full items-center justify-center">
          <div className="text-center">
            <div className="mx-auto flex h-14 w-14 items-center justify-center rounded-full bg-blue-50 text-blue-500 shadow-sm">
              <UsersRound className="h-6 w-6" />
            </div>
            <div className="mt-3 text-sm font-medium text-slate-700">暂无角色</div>
          </div>
        </div>
      ) : (
