    ...
    <div className="px-3 pb-1 shrink-0">
      <div className="flex items-end gap-2 bg-muted/30 rounded-2xl border px-2 py-2">
        <textarea
          ...
          onInput={handleInput}
          className="flex-1 bg-transparent resize-none text-sm leading-relaxed outline-none placeholder:text-muted-foreground/60 disabled:text-muted-foreground/40 py-2 px-2 min-h-[30px] max-h-[180px]"
        />
        <button
          disabled={disabled}
          onClick={handleClick}
          className="w-[52px] h-[36px] min-w-[52px] flex items-center justify-center rounded-xl bg-gradient-to-br from-amber-400 to-amber-600 text-white text-base shadow-md shadow-amber-500/20 transition-all hover:-translate-y-px hover:shadow-lg hover:shadow-amber-500/30 disabled:bg-muted disabled:text-muted-foreground/40 disabled:shadow-none disabled:hover:translate-y-0 shrink-0"
        >
          ↑
        </button>
      </div>
