...
function referenceAnchors() {
  ...
  function listReferenceMaterializationChapterProgress(input = {}) {
    ...
    const items = Array.from({ length: run.total_chapters }, (_, index) => {
      ...
      // 运行中的 run 把"下一章"标为进行中：让"正在处理第 N 章 · 阶段 · 轮次"
      // 与停滞告警有真实可断言的数据（失败章节数量不受影响）。
      const inFlight = run.status === 'running' && chapterIndex === Math.min(run.processed_chapters + 1, run.total_chapters)
      // 尚未轮到的章节是"等待处理"，与真实 run 的 processed_chapters 对齐（失败/进行中优先）。
      const status = failed ? 'failed' : inFlight ? 'llm_qualifying' : chapterIndex > run.processed_chapters ? 'pending' : 'completed'
      return {
        chapter_index: chapterIndex,
        batch_index: index,
        status,
        current_stage: status,
        candidate_count: status === 'pending' ? 0 : 6,
        decided_count: status === 'completed' ? 6 : failed ? 5 : inFlight ? 3 : 0,
        accepted_count: status === 'completed' ? 4 : failed ? 2 : inFlight ? 3 : 0,
        rejected_count: status === 'completed' ? 2 : failed ? 3 : 0,
        review_count: 0,
        vector_count: status === 'completed' ? 4 : failed ? 1 : 0,
        model_call_count: status === 'pending' ? 0 : 1,
        started_at: status === 'pending' ? null : now,
        completed_at: status === 'completed' ? now : null,
        last_error_code: failed ? 'materialization_llm_request_interrupted' : null,
        last_error_message: failed ? `第 ${chapterIndex} 章连接在生成完成前被切断（mock）` : null,
        row_version: 1,
        extraction_round_index: inFlight ? 2 : null,
        extraction_round_count: inFlight ? 4 : null,
      }
    })
