...
async function verifyChapterReferenceRetirementWorkflow(page) {
  ...
  // 章节参考抽屉已随拼装线退役（检索注入迁入聊天面板之前，编辑器不再挂参考面板入口）。
  await expectHidden(page.getByTitle('打开章节参考素材'), 'retired chapter reference drawer trigger')
  await expectHidden(page.getByTestId('chapter-reference-panel'), 'retired chapter reference panel')

  const assemblyCalls = await page.evaluate(() =>
  ...
  assert.deepEqual(assemblyCalls, [], 'chapter editor must not invoke retired assembly-line bridge methods')
...
export async function runFullSuite(browser, url) {
  ...
  if (runConfig.grep === '@chapter-reference') {
    logStep('checking chapter reference assembly retirement')
    await verifyChapterReferenceRetirementWorkflow(page)
  }
  ...
  // 风格素材 / 叙事模式面板已在轻量化聚焦 Phase 1 断入口退役（docs/corpus-driven-writing/lightweight-refocus-proposal-2026-08-31.md §8.2）。
  // verifyStyleSampleWorkflow / verifyChapterRangeSelectorWorkflow 随 Phase 2 删除组件时一并移除。
...
export async function runUsabilitySuite(browser, url) {
  ...
  await expectVisible(page.getByRole('heading', { name: '选择一个参考来源' }), 'usability corpus workspace heading')
  await expectVisible(page.getByTestId('reference-book-sidebar'), 'usability corpus book sidebar')
  await page.screenshot({ path: path.join(outputDir, 'usability-06-reference.png'), fullPage: true })
  observations.push(usabilityObservation({
    surface: 'Corpus / Materialization',
    severity: 'medium',
    issueType: 'ergonomic-friction',
    summary: 'The corpus workspace is reachable, but the materialization funnel still needs full task scoring against the lightweight refocus loop.',
    screenshot: 'usability-06-reference.png',
    reproduction: ['Open the initialized workspace.', 'Click the corpus activity (素材库).', 'Inspect the reference-book sidebar and materialization funnel.'],
    expected: 'A writer should register a reference book and see clear automatic segmentation, materialization progress, and review feedback with minimal ceremony.',
    actual: 'The surface is reachable and exposes source registration plus the split-materialize-review funnel, but the loop has not yet been scored with real reference books.',
    impact: 'Medium user impact for long-form writers; unclear automation would make corpus building feel like setup work instead of writing assistance.',
    proposedFix: 'Score the corpus funnel with real books: registration, automatic split, background analysis, card review, and writing-time recall surfaced in chat.',
