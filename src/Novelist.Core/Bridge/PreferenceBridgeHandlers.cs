...
public static class PreferenceBridgeHandlers
{
    public static BridgeDispatcher RegisterPreferenceHandlers(
        ...
        IPreferenceService service)
    {
        ...
        dispatcher.Register("GetPreferences", async (context, cancellationToken) =>
            await service.GetPreferencesAsync(ReadLongArg(context.Payload, 0, "novelId"), cancellationToken));

        dispatcher.Register("CreatePreference", async (context, cancellationToken) =>
            await service.CreatePreferenceAsync(
                ReadLongArg(context.Payload, 0, "novelId"),
                ReadObjectArg<CreatePreferencePayload>(context.Payload, 1, "input"),
                cancellationToken));

        dispatcher.Register("UpdatePreference", async (context, cancellationToken) =>
            await service.UpdatePreferenceAsync(
                ReadLongArg(context.Payload, 0, "preferenceId"),
                ReadObjectArg<UpdatePreferencePayload>(context.Payload, 1, "input"),
                cancellationToken));

        dispatcher.Register("DeletePreference", async (context, cancellationToken) =>
        {
            await service.DeletePreferenceAsync(ReadLongArg(context.Payload, 0, "preferenceId"), cancellationToken);
            return null;
        });
