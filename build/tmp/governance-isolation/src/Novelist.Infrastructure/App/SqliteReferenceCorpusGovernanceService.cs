...
public async ValueTask<ReferenceCorpusDedupResultPayload> RebuildDedupGroupsAsync(RebuildReferenceCorpusDedupGroupsPayload input, CancellationToken cancellationToken)
 ...
 public async ValueTask<bool> RecordInsertionAuditAsync(RecordReferenceCorpusInsertionAuditPayload input, CancellationToken cancellationToken)
 {
 ArgumentNullException.ThrowIfNull(input);
 if (input.NovelId <= 0 || input.ChapterNumber <= 0 || input.MaxSimilarity is < 0 or > 1) throw new ArgumentOutOfRangeException(nameof(input));
 return await LockedAsync(async connection => await ExecuteAsync(connection,
 "INSERT INTO reference_insertion_audits(audit_id,session_id,novel_id,chapter_number,candidate_id,assembled_text_hash,source_anchor_ids_json,max_similarity,gate_passed,diagnostics_json,created_at) VALUES($id,$session,$novel,$chapter,$candidate,$hash,$anchors,$similarity,$passed,$diagnostics,$time) ON CONFLICT(audit_id) DO NOTHING;",
 cancellationToken, ("$id", Required(input.AuditId, nameof(input.AuditId))), ("$session", Required(input.SessionId, nameof(input.SessionId))),
 ("$novel", input.NovelId), ("$chapter", input.ChapterNumber), ("$candidate", Required(input.CandidateId, nameof(input.CandidateId))),
 ("$hash", Required(input.AssembledTextHash, nameof(input.AssembledTextHash))), ("$anchors", System.Text.Json.JsonSerializer.Serialize(input.SourceAnchorIds)),
 ("$similarity", input.MaxSimilarity), ("$passed", input.GatePassed ? 1 : 0), ("$diagnostics", System.Text.Json.JsonSerializer.Serialize(input.Diagnostics)),
 ("$time", DateTimeOffset.UtcNow.ToString("O"))) > 0, cancellationToken);
 }
