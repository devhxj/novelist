...
await ExecuteAsync(connection,
 ...
 command.CommandText = $"SELECT member.library_id,member.anchor_id,observation.feature_family,NULL FROM reference_library_members AS member JOIN reference_feature_observations AS observation ON observation.anchor_id=member.anchor_id WHERE member.library_id IN ({string.Join(",", names)}) AND observation.validity_state='active' UNION ALL SELECT member.library_id,member.anchor_id,NULL,specimen.technique_family FROM reference_library_members AS member JOIN reference_technique_specimens AS specimen ON specimen.source_anchor_id=member.anchor_id WHERE member.library_id IN ({string.Join(",", names)}) AND specimen.validity_state='active';";
 for (var index = 0; index < libraryIds.Count; index++) command.Parameters.AddWithValue(names[index], libraryIds[index]);
 await using var reader = await command.ExecuteReaderAsync(cancellationToken);
 while (await reader.ReadAsync(cancellationToken)) sources.Add(new(reader.GetString(0), reader.GetInt64(1), reader.IsDBNull(2) ? null : reader.GetString(2), reader.IsDBNull(3) ? null : reader.GetString(3)));
 return sources;
 }
...
private static string BuildSummary(string type, IReadOnlyList<AggregateSource> sources) => $"{AggregateName(type)}：覆盖 {sources.Select(source => source.AnchorId).Distinct().Count()} 个来源，共 {sources.Count} 条有效证据。";
...
private static string Hash(string value) => Convert.ToHexString(SHA256.HashData(Encoding.UTF8.GetBytes(value))).ToLowerInvariant()[..24];
 ...
 private sealed record AggregateSource(string LibraryId, long AnchorId, string? FeatureFamily, string? TechniqueFamily);
